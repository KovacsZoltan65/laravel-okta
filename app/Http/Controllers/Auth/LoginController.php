...
// insert  user        
        try {
            $response = Http::withHeaders([
                    ...
                    ->withBody(json_encode(["profile" => [
                            "firstName" => $new_user['first_name'],
                            "lastName" => $new_user['last_name'],
                            "email" => $new_user['email'],
                            "login" => $new_user['email']],
                        "credentials" => ["password" => ["value" => $new_user['password']]]]))
...
// get user id for assigned to app  
        try {
            $response = Http::withHeaders([
                        "Accept" => "application/json",
                        "Content-Type" => "application/json",
...
// assigned user to app        
        try {
            $response = Http::withHeaders([
                        "Accept" => "application/json",
                        "Content-Type" => "application/json",
                        "Authorization" => "SSWS " . $this->_api_token_secret])
                    ->withBody(json_encode(["id" => $response_data['id'],
                        "scope" => "USER",
